        ...
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h4 className="text-2xl font-bold text-gray-800">
              {showConfirmation ? 'Vehicle Properties' : formSubmitted ? 'Choose a Vehicle' : 'Find a Ride'}
            </h4>
            {!loading && !formSubmitted && (
              ...
              >
                <ArrowDown
                  className={`transition-transform duration-500 ${panelOpen ? 'rotate-180' : ''}`}
                />
              </button>
            )}
          </div>
          ...
          <div
            ref={vehiclesRef}
            className={`space-y-4 ${!formSubmitted || showConfirmation ? 'hidden' : ''}`}
          ...
          <div
            ref={confirmRideRef}
            className={`space-y-4 ${!showConfirmation ? 'hidden' : ''}`}
            style={{ opacity: 0, transform: 'translateY(20px)' }}
          >
            <ConfirmVehicle
          ...
          <div className="p-4 bg-gray-50 border-t">
            <LocationPanel
              onSelectLocation={(loc) =>
                focusedInput === 'pickup' ? setPickup(loc) : setDestination(loc)
              }
            />
          </div>
        )}
